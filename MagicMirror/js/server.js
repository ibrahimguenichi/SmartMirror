...
function Server(config) {
  ...
  this.open = function () {
    return new Promise((resolve) => {
      ...
      // 1. Serve static files for React
      app.use("/home", express.static(path.resolve(global.root_path + "/modules/react-microfrontend/dist")));

      // 2. Serve index.html for any client-side route under /home/*
      app.get("/home/*", (req, res) => {
        const reactIndexPath = path.resolve(global.root_path + "/modules/react-microfrontend/dist/index.html");
        if (fs.existsSync(reactIndexPath)) {
          res.sendFile(reactIndexPath);
        } else {
          res.status(404).send("React app not found. Please build the React app first.");
        }
      });
